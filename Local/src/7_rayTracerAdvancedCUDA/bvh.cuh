    ...
    */
    ...
    while (stackTop > 0) {
        ...
        if (currentNode->_last_level) {
            ...
            if (currentNode->_left->hit(r, ray_t, rec)) {
                hit_anything = true;
                ray_t.max = rec.t; 
            }
            
            if (currentNode->_right->hit(r, ray_t, rec)) {
                hit_anything = true;
                ray_t.max = rec.t;
            }
        }
        else {
            stack[stackTop++] = (const bvh_node*)currentNode->_left;
            stack[stackTop++] = (const bvh_node*)currentNode->_right;
        }
    }
    return hit_anything;
}
...
__device__ bvh_node::bvh_node(hittable** l, int n, curandState* localState) {
    // TODO: Analyze and see the recursive 
    // version for tree construction. Try to implement an iterative version here using an explicit stack, before peeking at
     // Recursive approach
    /*
